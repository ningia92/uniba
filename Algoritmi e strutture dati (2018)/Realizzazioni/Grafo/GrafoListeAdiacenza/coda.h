...
template<typename T> bool Queue<T>::belongingQueue(elemtype e)
{
	Queue<T> supp;
	bool flag = false;
	while(!emptyQueue())
	{
		supp.enQueue(readQueue());
		deQueue();
	}
	while(!supp.emptyQueue())
	{
		if(e == supp.readQueue())
			flag = true;
		enQueue(supp.readQueue());
		supp.deQueue();
	}
	return flag;
}
