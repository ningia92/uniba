...
public:
	...
	position elemPos(elemtype) const;
private:
	position list;
};
...
template<typename T> bool PointerList<T>::searchList(elemtype element)
{
	bool found = false;
	if(!emptyList())
	{
...
template<typename T> typename PointerList<T>::position PointerList<T>::elemPos(elemtype e) const
{
	position p = NULL;
	if(!emptyList())
	{
		...
		cout << "Elemento non presente o lista vuota!\n";
