...
//	bool belongingTree(Node, Node) const;
	...
	void inOrderVisit(Node) const; // visita simmetrica
...
template<typename T> void Tree<T>::printTree(Node n) const
{
	Node tmp;
	if(n == getRoot())
		cout << "(NODE Root " << n << ", value: " << n->getElement() << ") --> (First son: " << n->getFirstSon() << ")\n";
	else
		cout << "(NODE " << n << ", value: " << n->getElement() << ") --> Parent: (" << n->getParent() << "), First son: (" << n->getFirstSon() << "), Next brother: (" << n->getNextBro() << ")\n";
	if(!leaf(n))
	{
		tmp = getFirstSon(n);
		printTree(tmp);
